...
class VGG16GraderCorners(VGG16GraderBase):
    ...
    def _define_remain_layer(self, inputs):
      self._remains_conv_0 = tf.keras.layers.Conv2D(8, (3,3), activation = 'relu')(inputs)
      ...
      self._remains_conv_1 = tf.keras.layers.Conv2D(8, (3,3), activation = 'relu')(self._remains_dropout)
    ...
    def _define_meaty_layer(self, inputs):
        self._dense_0 = tf.keras.layers.Dense(8, activation = 'relu', kernel_regularizer = tf.keras.regularizers.l2(0.01))(inputs)
        self._dense_1 = tf.keras.layers.Dense(8, activation = 'relu', kernel_regularizer = tf.keras.regularizers.l2(0.01))(self._dense_0)
        self._dense_2 = tf.keras.layers.Dense(8, activation = 'relu', kernel_regularizer = tf.keras.regularizers.l2(0.01))(self._dense_1)
