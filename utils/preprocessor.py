...
class UNETPreProcessor(object):
	...
	def process(self, img_path, isTrain=True):
		...
		front_shape = front_img.shape
		back_shape = back_img.shape

		front_img = cv2.resize(front_img, (1860, 3147), interpolation = cv2.INTER_AREA)
		back_img = cv2.resize(back_img, (1860, 3147), interpolation = cv2.INTER_AREA)
		...
		front_points = np.array([[pt[0],pt[1]] \
			 for pt in json.load(open(os.path.join(img_path, "front.json")))['shapes'][0]['points']])
		back_points = np.array([[pt[0],pt[1]] \
			 for pt in json.load(open(os.path.join(img_path, "back.json")))['shapes'][0]['points']])
		
		front_points = self.align_pts(front_points, front_shape).astype(np.int0)
		back_points = self.align_pts(back_points, back_shape).astype(np.int0)
	...
	def align_pts(self, pts, shape):
		for i in range(len(pts)):
			pts[i][0] = round(pts[i][0]*1860/shape[1])
			pts[i][1] = round(pts[i][1]*3147/shape[0])
		return pts
...
class VGG16PreProcessor(object):
	...
	def crop_image(self, image : np.ndarray):
		...
		# cv2.imshow("origin",cv2.resize(image, (image.shape[1] //4, image.shape[0] //4 ), interpolation = cv2.INTER_AREA))
		cv2.imshow("img", cv2.resize(origin_image, (origin_image.shape[1] * 2, origin_image.shape[0] * 2 ), interpolation = cv2.INTER_AREA).astype(np.uint8))
